...
### 步骤1：判断行情阶段
根据冶炼利润、库存水平、开工率判断当前处于哪个阶段：供给约束 / 供需平衡 / 供过于求 / 需求驱动
→ 根据阶段设定权重：供给(0.15-0.40) / 库存(0.15-0.25) / 需求(0.10-0.20) / 资金(0.10-0.15)

### 步骤2：核心矛盾识别
从18个指标中找出当前最核心的1-2个矛盾（边际变化大+价格弹性高），用"指标值→因果链→影响方向"格式。

### 步骤3：交叉验证
核心矛盾 → L2辅助指标交叉验证 → 强验证/弱验证/无法验证
